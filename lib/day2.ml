...
let parse_set acc set =
  ...
  let dice_defs =
    List.map (fun s -> String.split_on_char ' ' (String.trim s)) dice_defs
  in
...
let parse_dice s =
  ...
  {
    red = get_or_default "red" dice;
    green = get_or_default "green" dice;
    blue = get_or_default "blue" dice;
  }
...
let game_is_valid game =
  game.dice.red <= 12 && game.dice.green <= 13 && game.dice.blue <= 14
...
let part_2_solution =
  let games = List.map parse_line day_2_data_lines in
  let powers =
    List.map (fun g -> g.dice.red * g.dice.green * g.dice.blue) games
  in
  Utils.sum powers
